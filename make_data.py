...
from src.prepro.preprocessor_kr import (
    korean_sent_spliter,
    preprocess_kr,
    preprocess_kr_doc,
)
...
def _df_to_json(df, src_name, tgt_name, tgt_type, to_dir, subdata_group):
    ...
    for start_idx in tqdm(start_idx_list):
        ...
        for _, row in df.iloc[start_idx:end_idx].iterrows():
            src_sents = row[src_name] if isinstance(row[src_name], list) else korean_sent_spliter(row[src_name])
            # preprocessing!!!!! 아래 tgt도 살려주기
            # src_sents = [preprocess_kr(sent) for sent in src_sents]

            #############
            # print(src_sents)
            # sent_concat = "###".join(src_sents)
            # sent_concat = preprocess_kr_doc(sent_concat)
            # src_sents = sent_concat.split("###")
            # print(src_sents)

            # original_sents_list = []
            # for sent in src_sents:
            #     sent = " ".join(morpher.morphs(sent))
            #     sent = unused_tok_rpat.sub(r"[unused\1]", sent)
            #     original_sents_list.append(sent)
            # original_sents_list = [sent.split() for sent in original_sents_list]

            original_sents_list = [sent.split() for sent in src_sents]
            ...
            if subdata_group in ["train", "valid"]:
                if tgt_type == "idx_list":
                    ...
                    tgt_sents = row[tgt_name] if isinstance(row[tgt_name], list) else korean_sent_spliter(row[tgt_name])

                    # tgt_sents = [preprocess_kr(sent) for sent in tgt_sents]

                    #############
                    # sent_concat = "###".join(tgt_sents)
                    # sent_concat = preprocess_kr_doc(sent_concat)
                    # tgt_sents = sent_concat.split("###")

                    # tgt_sents = [sent.split() for sent in tgt_sents]
                    # summary_sents_list = []
                    # for sent in tgt_sents:
                    #     sent = " ".join(morpher.morphs(sent))
                    #     sent = unused_tok_rpat.sub(r"[unused\1]", sent)
                    #     summary_sents_list.append(sent)
                    # summary_sents_list = [sent.split() for sent in summary_sents_list]
                    summary_sents_list = [sent.split() for sent in tgt_sents]
