...
def validate(args, device_id, pt, step):
    ...
    valid_iter = data_loader.Dataloader(
        args, load_dataset(args, "valid", shuffle=False), args.batch_size, device, shuffle=False, is_test=False
    )

    tokenizer = BertTokenizer.from_pretrained(args.model_name, do_lower_case=True, cache_dir=args.temp_dir)
    symbols = {
...
def test_abs(args, device_id, pt, step):
    ...
    test_iter = data_loader.Dataloader(
        args, load_dataset(args, "test", shuffle=False), args.test_batch_size, device, shuffle=False, is_test=True
    )
    tokenizer = BertTokenizer.from_pretrained(args.model_name, do_lower_case=True, cache_dir=args.temp_dir)
    symbols = {
        "BOS": tokenizer.vocab["[unused0]"],
...
def baseline(args, cal_lead=False, cal_oracle=False):
    test_iter = data_loader.Dataloader(
        args, load_dataset(args, "test", shuffle=False), args.batch_size, "cpu", shuffle=False, is_test=True
    )
...
def train_abs_single(args, device_id):
    ...
    def train_iter_fct():
        return data_loader.Dataloader(
            args, load_dataset(args, "train", shuffle=True), args.batch_size, device, shuffle=True, is_test=False
        )
    ...
    train_loss = abs_loss(
        model.generator, symbols, model.vocab_size, device, train=True, label_smoothing=args.label_smoothing
    )
