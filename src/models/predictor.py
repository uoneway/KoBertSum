...
class Translator(object):
    ...
    def __init__(self, args, model, vocab, symbols, global_scorer=None, logger=None, dump_beam=""):
        self.logger = logger
        ...
        if self.beam_trace:
            self.beam_accum = {"predicted_ids": [], "beam_parent_ids": [], "scores": [], "log_probs": []}
        ...
        with torch.no_grad():
            for batch in data_iter:
                ...
                for trans in translations:
                    pred, gold, src = trans
                    pred_str = (
                        pred.replace(self.tgt_bos, "")
                        .replace("[unused3]", "")
                        .replace("[PAD]", "")
                        .replace(self.tgt_eos, "")
                        .replace(r" +", " ")
                        .replace(f" {self.tgt_sent_split} ", "<q>")
                        .replace(self.tgt_sent_split, "")
                        .strip()
                    )
                    gold_str = gold.strip()
                    if self.args.recall_eval:
